...
class AdapterResilience(unittest.TestCase):
    ...
    def test_dependency_and_identity(self):
        self.assertEqual(self.manifest["install"]["dir"], "zed_modules")
        if self.profile["releaseState"] == "blocked-until-certified-package-published":
            self.assertNotIn("opto-sync/opto-sync-clients", self.manifest.get("dependencies", {}))
            self.assertIn("opto-sync/opto-sync-clients@^0.4.0", (ROOT / ".zpkg.toml").read_text(encoding="utf-8"))
        self.assertEqual(self.profile["dependency"], {"package": "opto-sync/opto-sync-clients", "range": "^0.4.0", "installRoot": INSTALL})
        self.assertEqual(self.profile["repository"], os.environ.get("GITHUB_REPOSITORY", self.profile["repository"]))
